...
1. **Gemini API** (near-free, ~$0.01/video) — call directly via `curl`/Bash. The key lives in `.env` (git-ignored, never commit it) — load it with `export $(grep GEMINI_API_KEY .env)` before calling `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent?key=$GEMINI_API_KEY`. Use `gemini-3.6-flash`, NOT `gemini-2.5-flash` — that model is deprecated for new users (404) as of Sept 2026; Google's own model list may still be lag-listing it as available, don't trust that, use 3.6-flash directly. Pass a `file_data` part with the video's YouTube URL as `file_uri` to analyze it. No MCP needed. **This is the default analysis tool** — confirmed working and cheapest; try this before Higgsfield.
